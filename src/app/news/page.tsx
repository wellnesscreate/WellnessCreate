...
//      return output;
//    }
const API_URL = 'https://script.google.com/macros/s/AKfycbz9B-kHNoiypf60UVJNfsksNlzkn34xhqvzJG3dggh58NOevg1G0UukDlT_Usg5yKy6/exec';
  ...
  const fetchNewsData = async () => {
    ...
    try {
      ...
      try {
        ...
        const data: { success: boolean; data: NewsItem[]; error?: string } = await response.json();
        ...
        setError('APIからデータを取得できませんでした。デモデータを表示します。');
    ...
    return [
      {
        ...
        date: '2024年9月6日',
        summary: '株式会社WellnessCreateを設立いたしました。「Life style」に「Wellness」を浸透させることをビジョンに掲げ、スポーツメディア事業を展開してまいります。',
        content: '株式会社WellnessCreateを設立いたしました。「Life style」に「Wellness」を浸透させることをビジョンに掲げ、スポーツメディア事業を展開してまいります。',
      },
      {
        id: 2,
        category: 'media',
        ...
        date: '2025年2月1日',
        summary: '株式会社CANLIFEと代理店契約を締結いたしました。今後、両社の強みを活かしたサービス展開を予定しています。',
        content: '株式会社CANLIFEと代理店契約を締結いたしました。今後、両社の強みを活かしたサービス展開を予定しています。',
      },
      {
        id: 3,
        category: 'event',
        ...
        date: '2025年4月15日',
        summary: '東京都内にてスポーツイベントを開催いたします。詳細は後日発表いたします。',
        content: '東京都内にてスポーツイベントを開催いたします。詳細は後日発表いたします。',
      },
      {
        id: 4,
        category: 'service',
        ...
        date: '2025年3月16日',
        summary: '当社ウェブサイトをリニューアルいたしました。今後もコンテンツの充実を図ってまいります。',
        content: '当社ウェブサイトをリニューアルいたしました。今後もコンテンツの充実を図ってまいります。',
      },
    ];
  };
    ...
    <div className="bg-gray-50 py-16">
      <div className="container mx-auto px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-gray-600 mb-12">株式会社WellnessCreateからの最新情報をお届けします</p>
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            <button 
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium ${
                selectedCategory === null ? 'bg-gray-800 text-white' : 'bg-white text-gray-800 border border-gray-300 hover:bg-gray-100'
              }`}
            >
              すべて
            </button>
            {Object.entries(categoryLabels).map(([key, label]) => (
              <button 
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium ${
                  selectedCategory === key ? 'bg-gray-800 text-white' : 'bg-white text-gray-800 border border-gray-300 hover:bg-gray-100'
                }`}
              >
                {label}
              </button>
                  ...
                  <div key={item.id} className="bg-white rounded-lg shadow-md overflow-hidden">
                    <div className="p-6">
                      <div className="flex items-center mb-4">
                        <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${categoryColors[item.category]}`}>
